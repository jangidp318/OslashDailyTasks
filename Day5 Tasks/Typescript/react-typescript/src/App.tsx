...
export default class PokeList extends React.Component<{}> {
  state: PokemonListState = { page_num: 0, pokemon_list: null };
  ...
  componentDidUpdate(prevProps: {}, prevState: PokemonListState) {
    ...
    return (
      <div>
        {this.state.page_num >= 1 && (
          <button
            onClick={() => {
              this.setState((state:{page_num:number}) => ({ page_num: state.page_num - 1 }));
            }}
          >
            Prev
          </button>
        ...
        <ul>
          {this.state.pokemon_list?.map((pokemon: {url: string}) => (
